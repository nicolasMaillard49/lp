...
import { stepsR2, TOTAL_STEPS_R2 } from "@/config/form-r2";
...
function sanitizeAnswers(input: unknown): Record<string, unknown> {
  ...
  for (const step of stepsR2) {
    if (!(step.key in o)) continue;
    const raw = o[step.key];
    if (raw === null || raw === undefined || raw === "") continue;
    if (step.type === "scale") {
      const n = Number(raw);
      if (Number.isFinite(n)) out[step.key] = Math.max(0, Math.min(10, Math.round(n)));
...
export async function POST(req: NextRequest) {
  ...
  const lastStep = Number.isFinite(lastStepRaw)
    ? Math.max(0, Math.min(TOTAL_STEPS_R2, Math.round(lastStepRaw)))
    : 0;
  ...
  if (event === "submit") {
    ...
    payload.last_step = TOTAL_STEPS_R2;
